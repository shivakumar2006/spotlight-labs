...
const DashBoard = () => {
  ...
  const menu = [
    ...
    { name: "Contacts", path: "/dashboard/contacts", icon: <RiContactsLine className='text-2xl' /> },
  ];
  ...
  return (
    <div className='w-63 h-screen border-gray-200 border flex flex-col items-center'>
      ...
      {/* Sidebar Menu */}
      <div className='w-63 flex flex-col items-center gap-2 mt-5'>
        ...
        {menu.map((item, index) => {
          const isActive = (item.path === '/dashboard/home' && location.pathname === '/dashboard') || location.pathname === item.path;
